...
def Raw_Data_Processing():
    ...
    for root, dirs, files in os.walk(directory):
        ...
        fp.write(directory+r'\Stock_Market_Dataset.parquet', output, compression = 'GZIP')
    

# %%
#Problem 2: Feature Engineering
def Feature_Engineering():
    if os.path.exists(rf'c:\Users\Wu\Desktop\book\stock dataset\Stock_Market_Dataset.parquet'):
        directory = rf'c:\Users\Wu\Desktop\book\stock dataset'
        output_new = pd.read_parquet(directory+'\Stock_Market_Dataset.parquet')
...
def Integrate_ML_Training():
    if os.path.exists(rf'c:\Users\Wu\Desktop\book\stock dataset\Stock_Market_Dataset.csv'):
        directory = rf'c:\Users\Wu\Desktop\book\stock dataset'
        data = pd.read_csv(directory+'\Stock_Market_Dataset.csv')
        ...
        filename = directory+'\finalized_model.sav'
...
import ast
def Model_Serving():
    if os.path.exists(rf'c:\Users\Wu\Desktop\book\stock dataset\finalized_model.sav'):
        app = Flask(__name__)
...
with DAG(
   ...
   schedule_interval=None,
    ...
    #DAG_Raw_Data_Processing = Raw_Data_Processing()
    #DAG_Feature_Engineering = Feature_Engineering()
    #DAG_Integrate_ML_Training = Integrate_ML_Training()
    #DAG_Model_Serving = Model_Serving()
    ready = DummyOperator(task_id = 'ready')

    DAG_Raw_Data_Processing >> DAG_Feature_Engineering >> DAG_Integrate_ML_Training >> ready
